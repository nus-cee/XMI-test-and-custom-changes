...
4. Update or add unit tests under `test/` to cover library contract changes (e.g., serialization expectations, schema validation) and run `dotnet test`. **Done (manual harness)** – `test/StructuralSegmentMapperHarness.cs` plus the new `StructuralSegmentHarnessCommand` button exercise Line/Arc/Spline paths inside Revit without introducing an external test project. Future automation can still wrap these helpers once Revit stubs/offline packages are available.
