...
1. Audit the repository: inventory current branches, tags, CI workflows, and external assets (Revit DLL references in `RevitXmiExporter.csproj`).
...
2. Update `RevitXmiExporter.csproj` to reference the new package version and run `dotnet restore`.
