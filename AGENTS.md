...
## Testing Guidelines
Automated tests are still being bootstrapped; until a dedicated test project is added, validate JSON output locally using the data builders in `Test/` and Revit sample models. Name new tests after the component under test (`StoreyTests.BuildsExpectedStoryJson`). Once you add an xUnit or NUnit project, keep it under `Test/` and update the solution so `dotnet test` executes it in CI. Document any manual Revit steps (models used, parameters toggled) inside `/Test/README.md` when you add new cases.
